// http://localhost:8090/ch17/board/listArticles.do

package ch17.sec03.brd07;
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (action == null) {
				...
				request.setAttribute("article", articleVO);  // articleNO에 null이 들어갔는지 확인
				...
				session = request.getSession();                // parentNO에 널값이 들어갔는지 확인
			...
			} else if (action.equals("/addReply.do"))
			{
				...
				pw.print("<script>" + "  alert('답글을 추가했습니다.');" 
						...
						+ "/board/viewArticle.do?ariticleNO="
						+ articleNO +"';"+ "</script>");
				return;
